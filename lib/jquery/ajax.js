...
module.exports = exports = function($, exec, ajaxCache) {
      ...
      exec.exec(function() {
        ...
        try {
          ...
          if (!err && response.statusCode === 200) {
            options.success && options.success(ajaxCache ? clone(response.data) : response.data, type, xhr);
          } else {
  ...
  return request({
    ...
    function(err, response, body) {
      ...
      if (caching && !ret.cachingInfo['no-cache'] && !ret.cachingInfo.private) {
        callback(undefined, {
          getTtl: function() {
            return ret.cachingInfo.expires - Date.now();
          },
          toCache: function() {
            return ret;
          }
