...
module.exports = exports = function($, exec) {
  ...
  $.ajax = function(options) {
    ...
    function callback(err, response, body) {
      ...
      exec.exec(function() {
        ...
        try {
          ...
          xhr.responseText = (response && response.responseText) || body;
    ...
    var requestType = options.type || 'GET',
        cacheEntry = cache[options.url];
    if (requestType === 'GET' && cacheEntry) {
      process.nextTick(function() {
        callback(undefined, cacheEntry, clone(cacheEntry.data, true));
      });
      xhr.readyState = 4;
      return xhr;
    }
