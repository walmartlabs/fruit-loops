...
module.exports = exports = function(window, exec, options) {
  ...
  function generateFunc(url, next) {
    execRequest({url: url}, undefined, function(err, response) {
      if (err) {
        return next(err);
      }
  ...
  if (ajaxCache && !ajaxCache.getOrGenerate) {
    ajaxCache.rules({
  ...
  window.$.ajax = function(options) {
    ...
    function callback(err, response, cache, report) {
      ...
      exec.exec(function() {
        ...
        try {
          ...
          var cacheLookup = report && (report.error || report.msec);
    ...
    if (ajaxCache && (!options.type || options.type === 'GET')) {
      if (ajaxCache.getOrGenerate) {
        ajaxCache.getOrGenerate(options.url, function(next) { generateFunc(options.url, next); }, callback);
      } else {
        ajaxCache.get(options.url, callback);
      }

      // Do a custom timeout here so we can prime the cache for future requests
      startTimeout();
    } else {
      req = execRequest(options, timeout, callback);
      requests.push(xhr);
    }
...
function execRequest(options, timeout, callback) {
  var jsonp = false,
      url = options.url;
