...
module.exports = exports = function Ajax(window, exec, options) {
  ...
  window.$.ajax = function(options) {
    ...
    var timeout = Math.min(options.timeout || Infinity, globalTimeout || Infinity);
    if (timeout === Infinity) {
      timeout = undefined;
    }
    ...
    if (ajaxCache && (!options.type || options.type === 'GET')) {
      ajaxCache.getOrGenerate(options.url, function(callback) {
          execRequest(options, true, jsonp, timeout, callback);
        },
        callback);
    } else {
